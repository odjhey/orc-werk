...
- Work only inside `.worktrees/<branch>` under the repository root. Create it with `git worktree add .worktrees/<branch> -b <branch> master`.
- Never merge, never push to `master`, never run `gh pr merge`, never record an assurance verdict. Landing is the watchtower's act.
- The ledger lives at the primary checkout root: always run `orc` as `ORC_JOURNAL_DIR=<repo-root>/.orc uv run --project <repo-root> orc ...`.
