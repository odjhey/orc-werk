...
Additional unit coverage beyond the shared mixin (task card acceptance item): `tests/conformance/test_acp_execution_unit.py` — the three unobservability-determination branches (daemon-dead-no-result → `failed`; result-present → `settled`; alive-no-result → `running`, never a timeout) and cancel post-verification (both the in-flight-turn and nothing-to-cancel cases).
