...
### Queue-owner keep-alive (`execution.ttl`, issue #206)

Every invocation begins `acpx --format json --ttl <seconds> <agent> ...`; `--ttl` is a top-level acpx flag before the agent subcommand. The adapter-local `execution.ttl` option is a non-negative integer and defaults to `0`, meaning keep the queue owner alive forever. Emitting it on every invocation ensures whichever command spawns the owner applies the configured lifetime and avoids acpx's 300-second idle default killing an executor mid-turn. Session close/cleanup is deliberately not added here: issue #207 owns that lifecycle work because settle-time close would violate issue #57 replay idempotency.
...
This is a deliberate choice over the spike's alternative "recommended pattern" (foreground `pi -s <name> "<prompt>"`, blocking until the terminal NDJSON line). Observing the terminal `stopReason` does not prove that provider tail-end worktree writes are already quiescent; composition with `GitDiffCandidate` relies on that adapter's bounded two-stable-reads/no-`index.lock` identification rule before it binds a candidate. The ACP settlement itself is not delayed or reinterpreted.

The orchestrator's poll model (`PORT-EXEC-001` returns a stable ref; `PORT-EXEC-002` is polled separately, potentially from a different process after a restart) wants `start()` to be cheap and non-blocking:
...
The stream-scan (`_scan_stream_terminal_results`) finds lines shaped `{"id": <int>, "result": {"stopReason": "..."}}`, then applies a terminal-quiescence check. A recorded result is only a settlement candidate when every later record is positively identified as passive protocol bookkeeping. The passive allowlist is limited to `initialize` and `session/load` requests and their correlated non-`stopReason` responses: real successful Pi transcripts end at the turn result, while reconnects may append exactly those handshake records. Any later `agent_message_chunk`, `agent_thought_chunk`, `tool_call`, or `tool_call_update`, any new prompt, or any provider retry/backoff marker is continued activity and suppresses settlement. Any malformed or unknown later record is ambiguous and also suppresses settlement. This is issue #181's success-side application of issue #157's corroborate-before-settling precedent: ambiguous evidence maps to `running`, never guessed success.

The issue #181 record order — completed tool update, idle session info, `result.stopReason=end_turn`, then `agent_message_chunk` containing a retry/backoff notice — therefore does not settle. `inspect()` reports `running`, records suppression evidence under `acp-settlement/v1`, and leaves ordinary polling plus the corroborated daemon-death path to determine the eventual fate.
...
2. Settle `failed` only when no terminal `stopReason` has been recorded for the outstanding turn (per the correlation rule above) and death is corroborated by one of: (a) `sessions show` recording a nonzero integer `lastAgentExitCode` or an agent exit signal; (b) `status -s <name>` reporting literal `status == "dead"` with `pidAlive == false`; or (c), for issue #206, all four vanished-worker conditions hold: **(i)** no terminal result (guaranteed before the death check), **(ii)** the durable show record is prompted (`lastPromptAt` set or `messages` non-empty), **(iii)** status is exactly `"no-session"`, and **(iv)** the stream for the outstanding turn contains substantive agent/tool activity (`agent_message_chunk`, `agent_thought_chunk`, or `tool_call*`) after the latest prior terminal result. The resulting `acp-settlement/v1.unobservability` reason is `worker-vanished-mid-turn` and records the prompted/activity corroboration.
3. The activity condition is the startup-transient discriminator. A freshly prompted session may briefly report `no-session` while its queue owner starts, but its turn stream is empty or has no substantive activity; that evidence is ambiguous and remains `running`. This extends issue #157's rule: ambiguous evidence maps to `running`, never false failure. A lone `dead`, `no-session`, prompted marker, or old-turn activity is insufficient.
4. **Never a timeout.** There is no wall-clock/retry-count path to `failed` anywhere in `inspect()`. The preceding requirements are corroboration rules, not timers. Existing nonzero-exit, signal, and `dead` + `pidAlive == false` branches are unchanged.

A literal `"dead"` can describe a held lease whose IPC socket did not answer; it counts only with `pidAlive == false`. `dead` with a live/unknown owner remains `running`. Likewise, `no-session` without both prompted and current outstanding-turn stream activity remains `running`. This conservative behavior follows the #157 precedent: wait rather than guess.

At fix time, acpx 0.13.1's JSON output was re-probed via `acpx --format json pi status`: public fields use camelCase, but the installed CLI's `status_snapshot` omitted internal health fields `pidAlive` and `hasLease`. Such a payload is intentionally ambiguous under this rule; a future acpx version may expose `pidAlive`, which the adapter parses using that observed camelCase spelling. On the installed acpx, which omits `pidAlive`, a lone `dead` snapshot remains ambiguous. A vanished owner can still settle through issue #206's independently corroborated `no-session` plus prompted/current-turn-activity branch; without that full evidence it remains `running`.
...
| `PORT-EXEC-002 inspect` | `acpx <flags> <agent> sessions show <name>` + raw `eventLog.active_path` stream scan; `acpx <flags> <agent> status -s <name>` only when checking for daemon death | No terminal-quiescent `stopReason` and daemon alive/ambiguous → `state=running` (including a result followed by continued or ambiguous activity); terminal-quiescent `stopReason` present for the correlated turn → `state=settled`, `outcome` per the stopReason table below; no terminal-quiescent `stopReason` and daemon confirmed dead → `state=settled, outcome=failed` (unobservability). |
...
The registered adapter-local `acp-settlement/v1` extension is emitted as a sibling of `execution-session/v1`. Unobservability-determined `failed` settlements use its `unobservability` object for the portable JSON evidence that discriminated the observation. Running observations whose recorded result was suppressed by terminal-quiescence use its `suppression` object, identifying the candidate result and first non-passive later record. Keeping both diagnostics out of the published session-provenance schema is the issue #45 remedy: this evidence explains the adapter's observation, never overrides canonical state/outcome. The former undifferentiated `_orcw_unobservable` boolean remains removed.
...
7. Global/output flags (`--format`, `--ttl`, `--json-strict`, `--cwd`, `--approve-all`/`--non-interactive-permissions`) precede the agent subcommand (`_base_argv` always builds them before appending the agent token).
...
12. `acpx pi status`'s `"no-session"` value is death evidence only under issue #206's four-condition corroboration rule (no result, durably prompted, exact `no-session`, and substantive activity in the outstanding turn's stream). Without current-turn activity it is a startup transient and remains `running` per #157. Literal `"dead"` still requires `pidAlive == false`; `dead` with a live or unknown owner remains `running`.
