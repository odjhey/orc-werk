...
2. Settle `failed` **only** when both of these hold: no terminal `stopReason` has been recorded for the outstanding turn (per the correlation rule above), **and** daemon death is corroborated by either (a) `sessions show` recording a nonzero integer `lastAgentExitCode` or an agent exit signal, or (b) `acpx pi status -s <name>` reporting literal `status == "dead"` **and** its JSON payload reporting `pidAlive == false`. A lone `status == "dead"` observation is not sufficient evidence that the outcome is lost. `dead` with `pidAlive == true` means a lease is held while the IPC socket is momentarily unreachable (the queue-owner startup window or a busy owner), so it MUST remain `running`; a missing or unparseable `pidAlive` is likewise ambiguous and remains `running`. Exit code `0` alone is not death evidence: acpx's own `resolveStatusState` treats it as idle, not dead.
3. **Never a timeout.** There is no wall-clock/retry-count path to `failed` anywhere in `inspect()`. The preceding requirement is a corroboration rule, not a timer.
...
12. `acpx pi status`'s `"no-session"` value must **not** be treated as "daemon dead" — it is also returned for a session that has not yet spawned a queue owner (e.g. immediately after `ensure`, before any prompt). Nor may literal `"dead"` stand alone: it requires `pidAlive == false`; `dead` with a live or unknown owner remains `running`.
