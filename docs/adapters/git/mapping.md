...
- **`repo_path`** disambiguates candidates across repositories/worktrees, and is included by default. This adapter's target use is one Work per one configured worktree, so including it in the fingerprint does not create false negatives in that usage. `GitDiffCandidate(..., include_repo_path=False)` opts out for callers that want cross-path identity instead.

A race observation may additionally carry `subject_identity.extensions["git-candidate-identification/v1"]`. This is adapter-local provenance, not identity: fingerprinting excludes `extensions` and covers exactly `head_sha`, `diff_digest`, and `repo_path` when enabled. Thus marked and unmarked observations of the same Git subject have the same fingerprint. See `EXT-GIT-CANDIDATE-IDENTIFICATION-V1`.

## Fingerprinting

Reuses `orc_werk.adapters.scripted.candidate.fingerprint_of` — the same canonical-JSON sha256 helper every other adapter uses (sha256 over `sort_keys=True` JSON of the portable identity fields, truncated to 24 hex chars, `fp-` prefixed). `CONF-CAND-001`/`CONF-CAND-002` require deterministic identity content; adapter-local observation provenance is deliberately outside that content.
...
Before binding, `identify()` confirms a quiescent ref: two consecutive commit-resolution reads must agree, with a short bounded settle interval between them, and Git's resolved `index.lock` path must be absent. When a read advances, the later value becomes the next baseline. Confirmation is bounded to three comparisons. Exhaustion is **never a timeout-to-failure**: the adapter binds the latest observed commit. The diff is then read against that bound SHA, not the moving ref. Only when the final bound SHA differs from the initial observation does the adapter emit `git-candidate-identification/v1` and print `note: worktree advanced during candidate identification (<A>..<B>); bound <B>`. An ABA sequence ending at the initial SHA emits neither marker nor note because the bound outcome did not advance.

The settle interval gates **when** Git is observed; its duration and wall-clock time enter neither candidate data nor durable idempotency-key material (`INV-020` governs the latter). Stable observations therefore retain the pre-rule candidate shape byte-for-byte. A still-present lock after the bound degrades to bind-latest rather than failing, with no invented timing data.
...
Every call re-reads real git state; there is no cached identity to preserve beyond git's own commit/worktree state. Calling `identify()`/`current()` repeatedly against an unchanged worktree returns the same fingerprint every time (`CONF-CAND-001`). The CandidatePort performs no state-changing effect and therefore has no effect idempotency key governed by `INV-020`.
