...
| 7a. Concurrent `omp -p` invocations can silently exit `0` with no task output | OMP process-level concurrency (native `omp -p` launch, not the `task` subagent-spawn tool) | **FAIL** (report §7a): reproduced at N≥3 same-provider and N=4 mixed-provider — a lone `anthropic/claude-sonnet-5` process among four also died silently, ruling out a single-provider cause; `rc=0` alone is not proof of completion. Tracked as harness-runtime remediation separate from this adapter-docs reconciliation: issue #302, scoped to the observed print-mode launch path and explicitly not assumed to apply to native `task`-tool subagent spawns. |
| 7b. The executing model can silently differ from the requested one | `retry.fallbackChains` (rung-1 user-level config) substituting a model at spawn, with no error or user-visible notice in text mode | Gap confirmed (report §7b): `mapping.md`'s `executor-identity/v1.model` field mapping is self-reported and unverified — a seat can honestly report the model it *requested* while `retry_fallback_succeeded` silently ran a different one. Weakens `V7` (verify family ≠ ship family) auditability from the journal alone; tracked as issue #281. |
