...
| 2. Ship agent creates its own worktree; a hook fences writes outside it | `.omp/extensions/orc-seat.ts` — the sole guard the 2026-09-07 operator ruling retains in the hook; does not exist in this repository's `e102e1e` baseline (`TASK-M5-005`, PR #284, observed OPEN and unmerged at `2026-09-07T07:02:32Z`) | PASS for the fencing mechanism itself, red-then-green (report §2), with a corrected premise: OMP does not auto-create the worktree for a non-isolated spawn, so the hook must be told the path by a fixed convention, not read it off an OMP-tracked field |
