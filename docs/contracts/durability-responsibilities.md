...
| Structured crew reports (append-only handoff history) | no | adapter-owned execution/report log (watchtower recommendation on record) | `crew-report/v1` (schema proposed in issue #12; not registered) | **OPEN GATE — due at M1b design time**; `PRODUCT-ADOPTION`'s Rozoro worked example (`docs/product/adoption.md` §4) is persona-derived design input for this gate — it confirms the adapter-owned-log disposition and that crew "steering" maps onto `CAP-EXEC-SEND` |
| Report open-item / acknowledgement state | no | same adapter as crew reports | inside `crew-report/v1` or a companion contract (undecided) | **OPEN GATE — due at M1b design time**; when designed, rename the report's `verdict` field to `claimed_verdict` per the watchtower recommendation, to keep a crew "done" report legible as a claim (`INV-003`) and not confusable with canonical assurance (`EXT-003`) |
...
| `tasks/<id>/handoff.md` | append-only crew report history | delegated — **OPEN GATE M1b** | adapter-owned report log (recommendation on record) | `crew-report/v1` (proposed, not registered) | teardown/reload preserves the full report history — not yet defined |
| `.acked-blocks-v2` | unresolved/report ACK cursor | delegated — **OPEN GATE M1b** | same adapter as crew reports | report ACK contract (undecided: inside `crew-report/v1` or companion) | cursor/open items survive reload — not yet defined |
...
Per issue #12: unclassified durable objects (rows with no disposition, or a delegated row with no named owner at all) block the **Rozoro-replacement milestone** — the point at which Rozoro is retired as the operational system of record. They do **not** block M1. M1a/M1a+/M1b (`M-001`) may ship with `crew-report/v1`, the multi-work `work-spec` owner, the runtime evidence contract, and the attention contract still open, exactly as recorded above, because M1b's own acceptance bar (`docs/delivery/M1-delivery-ledger.md`) does not require closing those gates — it requires this contract to exist and the retirement ledger to have zero *unclassified* rows, which this document satisfies: every row above reaches canonicalized, delegated (registered, planned, or OPEN GATE), implementation-local, or intentionally dropped.
