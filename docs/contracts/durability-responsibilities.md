...
| Delegated work specification — multi-work brief | no | future WorkGraph adapter's native issue/description store (e.g. a Beads-backed `PORT-WORK-GRAPH` adapter) | none registered; deferred to the first real WorkGraph adapter | **resolved — adapter-owned** (operator ruling, 2026-08-28, issues #12/#47): the kernel does not journal per-work briefs and no dedicated artifact store will be built. **Until such an adapter exists, multi-work briefs are deliberately NOT durable** — a run's journal reconstructs its topology and every verdict, but not what each work meant; per-work specifications live in the dispatching party's records (ephemeral). The single-work case remains durably owned by `FACT-INTENT-SUBMITTED` (row above). Operators who need durable multi-work briefs today should carry the essentials in the run-level intent text. |
| Run topology (create plan: works + dependency edges) | no | kernel `PORT-JOURNAL` — the `FX-CREATE-WORK` effect record's journaled `data.plan` (`PORT-WORK-001` plan shape) | none registered; already satisfied by the effect record | **resolved — normative** (operator ruling, issue #41): a journal from which the run's topology cannot be reconstructed is non-conformant. Distinct from the still-**OPEN** multi-work brief-text row above — topology is the graph, brief is the prose spec. Presentation surfaces (e.g. the report's future dependency-tree view, issue #41) MAY rely on it. |
