...
| Record observation wall-clock times (per-journal-record `observed_at`) | no | JSONL journal adapter sidecar | this row (`CONTRACT-DURABILITY`) — adapter-owned sidecar, not a `CONTRACT-EXTENSIONS` envelope `extensions`-slot extension | **resolved** (issue #39; filename layout amended by issue #55 H1) — one append-stamped `{seq, observed_at}` line (ISO-8601 UTC) per journal record, beside — never inside — the run's own journal. Every run created under issue #55's per-run directory layout gets `<run_id>/times.jsonl` beside that same directory's `<run_id>/journal.jsonl`; a run that already existed before issue #55 keeps its legacy `<run_id>+times.jsonl` flat file beside `<run_id>.jsonl` for its whole lifetime instead (read/write-fallback, never a mid-run switch). This sidecar remains best-effort (`_stamp_observed_at` swallows `OSError`, never fails the canonical append it follows) and adapter-owned, not canonical orchestration truth — `TASK-FIX-285`'s `orc census` (issue #285) is now a second reader class beyond presentation (`orc report`/`orc show`'s render-only duration/timeline use): an aggregate *counting* surface, where an absent or corrupt entry changes what a dated tally includes, not merely what a timestamp column displays. Census inherits the same never-fabricate stance the presentation readers already established (a missing/corrupt entry is disclosed as `undated`/skipped, never guessed) rather than being granted any stronger durability promise than this row already makes.
