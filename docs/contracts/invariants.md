...
- `FX-START-ASSURANCE` (assurance-targeting) uses the standard tuple plus `candidate_fingerprint`, plus `assurance_number` (`INV-021`) when that number is greater than `1`. The first assurance of a candidate within an execution attempt keeps the pre-`INV-021` form (no `assurance_number` component), so every journal written before assurance re-request existed replays under identical keys (`ADR-0006`).

## INV-021 — Assurance re-request is bounded

An `inconclusive` assurance settlement is resolved by a finite assurance budget, never by fiat. Policy MUST define `max_assurance_attempts`, the finite number of assurances permitted for one Candidate within one execution attempt. While `assurance_number < max_assurance_attempts`, an `inconclusive` settlement MUST lead to a re-request of assurance for the same exact Candidate under a new assurance identity; when the budget is exhausted, it MUST resolve via `DEC-BLOCK`. `assurance_number` is the per-execution-attempt assurance index (first assurance `1`) and MUST be deterministically reconstructable from the journal as the count of `FACT-ASSURE-STARTED` records for the current Execution. Assurance re-requests MUST NOT consume the execution retry budget (`INV-018`/`INV-019`): they journal no execution-start record. The effective `max_assurance_attempts` MUST be journaled at run creation under the same single-authority rule as `max_attempts`; a journal that predates the field folds under a budget of `1`. See `STATE-DELIVERY` item 11, `SCN-021`, `ADR-0006`.
