...
## Outcome / scope
A repo sets its dispatch defaults once (a plain-JSON profile at .orc/profile.json, CLI-layer discovery only); precedence --config (deep-merged) > per-run persisted > .orc/profile.json > {}; onboard writes it idempotently. Reuses config.py _validate_* ; no core/app change. Design source: M4-COCKPIT-AND-CLARITY Phase M4a + open questions 1/2/5.
