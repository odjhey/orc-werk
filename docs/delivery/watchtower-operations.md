...
5. **Merge** — watchtower only, squash merges, branch refreshed against master first (the single required `ci-required` status check is strict). Doc amendments merge before the code they govern whenever possible.
   ...
   - **A `NON-BLOCKING` finding on PR prose is a merge-gate obligation, not an option (issue #295).** A verify verdict binds only to the diff at the derived sha — the candidate — never to the PR's own title/description text; editing that text does not change the fingerprint a verdict already judged, so a `NON-BLOCKING` finding about misleading or stale PR prose never triggers re-verification. It does, however, bind the watchtower: correct the prose before merging. Merging a candidate the ledger recorded as correct behind a description the same ledger already flagged as wrong is a defect, not a shortcut.
