...
An `unobservability` payload with `reason: "worker-vanished-mid-turn"` records the ACP adapter's corroborated issue #206 shape: no terminal result, a durable prompted marker, exact `no-session` status, and substantive agent/tool activity in the outstanding turn's stream. Empty/no-substantive stream activity is a startup-transient ambiguity and MUST remain running under the issue #157 precedent.
