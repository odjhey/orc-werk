...
The ACP adapter emits this extension only when it determines an execution settlement is `failed` because the outstanding result is unobservable under the ACP mapping's corroboration rule. It emits the extension beside, not inside, `execution-session/v1`.
