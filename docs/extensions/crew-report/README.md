...
The durable owner of the append-only crew-report history is an **adapter-owned append-only log, beside — not inside — the journal**: a plain NDJSON file per `DeliveryRun`, distinct from (and never merged into) the `JournalPort`'s own `<delivery_run_id>.jsonl` file. This is the watchtower-recorded disposition from issue #12 and `CONTRACT-DURABILITY`'s ownership matrix, now closed.
