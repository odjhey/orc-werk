...
description: Guidance for ship/verify subagents recording their own observations through the orc CLI (M1a+ push mode) — harness-independent seat discipline; OMP-specific mechanics live in docs/adapters/omp/.
---
...
2. **Do the work.**
3. **Record the outcome and candidate** for the config/backing store the ExecutionPort reads (see Mechanics below) — not prose. Prefer `orc record <run-id> --work <work-id> --outcome completed|failed ...` (the ship-seat sibling of the verify seat's `--verdict` path, mutually exclusive with it per invocation): it validates that the work is currently awaiting `execution-outcome` and performs the same merge-only atomic config update, then prints the exact resume command without running it. Add `--evidence-ref` (repeatable; rides the attempt entry's canonical `artifact_refs` field, transported losslessly into `FACT-EXEC-SETTLED.artifact_refs`) and `--model`/`--session-ref`/`--seat-ref` (become `executor-identity/v1` with `role: "ship"`, per §2). The verb never records candidate identity: with a `git` candidate adapter the next dispatch pass identifies the candidate itself; with a scripted candidate you still hand-author the entry's `candidate` payload (the verb merges `outcome` into that same slot). Candidate content MUST be **externally resolvable identity**: a PR number, a head sha, a run URL — anything a stranger with no access to your reasoning could independently fetch and check. Never record a description of what you did as the candidate; a sentence is not verifiable, an artifact reference is. Hand-editing the persisted config remains legal with identical semantics and discipline — the verb is validation sugar, not a new recording contract.
...
4. **Candidate-only blocking (issue #295).** The candidate you are verifying is the content covered by the candidate identity you derived (`PORT-CANDIDATE`'s `subject_identity`/`fingerprint` — provider-native and opaque to the core; for a Git-backed PR, the diff at the derived sha). A defect confined to content your adapter's fingerprint does NOT cover — for a Git-backed PR, its own mutable prose (title, description) — is real and MUST be reported — prefix it `NON-BLOCKING` in `findings` — but it MUST NOT by itself turn an otherwise-earned `accepted` into `rejected`: rejecting there would burn an attempt budget re-verifying a candidate you already confirmed is correct. Editing that uncovered content does not, and cannot, change the fingerprint you derived. `NON-BLOCKING` is not "ignore it" — for a Git-backed PR the watchtower is required to fix the prose before merging (`PLAYBOOK-WATCHTOWER`'s merge gate); it is only not a reason to withhold acceptance of the candidate itself.
5. **Record `inconclusive` when you cannot decide — never `rejected` for a failure that is yours.** `orc record --verdict inconclusive` is the honest verdict when (a) you evaluated the candidate and genuinely cannot decide (identity diverged from what was recorded, output ambiguous), or (b) you could not evaluate at all (sandbox down, tooling missing, timeout) and do not expect to retry within this seat. It costs the run assurance budget (`max_assurance_attempts`, `INV-021`), never the ship seat's retry budget: within budget the kernel re-requests assurance of the *same* candidate and the next verify seat picks it up (`SCN-021`); exhausted, the Work blocks with `reason: assurance-inconclusive` for the operator. Put the reason in `evidence_refs` (the log, the error) so the ledger shows *why* — that legibility is the point. For a transient outage you will retry yourself within minutes, recording nothing and re-running your audit is still fine (`STATE-DELIVERY` item 7: waiting is a legal resting point). Recording `rejected` for a verifier-environment failure is a defect: it spends the shipper's budget on a defect the candidate never had and triggers a corrective-intent round for nothing.
6. **Record `evidence_refs`** pointing at your audit output (the command you ran, the diff you fetched, the log you read) — not a narrative summary. Evidence is candidate-bound (`INV-007`) and non-transferable across candidates (`INV-008`): if the candidate changes, your prior evidence no longer applies and you must re-derive.
7. **Record the verify seat's identity when no adapter journals it.** Put the registered `executor-identity/v1` extension described in §2 alongside the verdict in the config assurance entry's `extensions` key, including `model`, `session_ref`, `seat_ref`, and `role` (for example, `"role": "verify"`). Choose a `seat_ref` stable for this verifier seat and distinct from the ship seat's value so the two identity payloads for this candidate are distinguishable from the journal alone; sharing an orchestrating `session_ref` does not establish or disprove seat separation. The payload transports unchanged into `FACT-ASSURE-SETTLED.extensions` per `CONF-EXT-003` and does not alter canonical assurance fields.
8. **Record the audit base when Git-backed.** A Git-backed verify seat SHOULD put an `assurance-context/v1` payload alongside the verdict in the config assurance entry's `extensions` key, with `base.identity` set to the resolved immutable sha the audit compared against (never a bare mutable ref). This is verifier-attested provenance, not a value the kernel re-derives or validates; see `EXT-ASSURANCE-CONTEXT-V1`.
9. **Journal substantive review findings with the verdict.** When your review produces substantive findings, put a `review-findings/v1` payload in the config assurance entry's `extensions` key. That entry accepts extensions and transports them losslessly into `FACT-ASSURE-SETTLED` per `CONF-EXT-003`; `EXT-REVIEW-FINDINGS-V1` is the schema home. A transcript path alone is not a durable record of what the review found.
10. As a standing discipline (SHOULD, not just for this one verdict): periodically reconcile the ledger against GitHub — every `ACCEPTED` Work's PR is actually merged, every recorded sha actually exists. This reconciliation-as-checker-duty is a supporting practice, not a one-time step.
...
- **One writer per run journal at a time.** Concurrent writers to the same run's journal are not supported; if two agents believe they own the same run, that is a coordination bug upstream of the CLI, not something the CLI arbitrates. **Amendment (`CONTRACT-STORAGE-CONCURRENCY` A3):** this rule remains binding seat semantics — who may advance a run — but is no longer this repo's correctness mechanism against storage corruption. The storage layer itself is required to stay safe under concurrent processes (`CONTRACT-STORAGE-CONCURRENCY` §13); a violation of one-writer-at-a-time is a coordination defect to fix upstream, not a corruption risk to guard against by convention alone.
- **Recording is a config/backing-store write, not a journal write.** Prefer the `orc record` verb for either seat's recording (full command shape, config schema, and the `--verdict`/`--outcome` mutual-exclusivity rule: `PLAYBOOK-CLI-USAGE`); hand-editing the persisted config remains legal with identical semantics and discipline. Either path, the kernel journals the resulting facts itself via the normal observation path on the next `orc dispatch` (`SCN-007`) — you never hand-author journal records.
- **Exit codes and what they obligate you to do next** (full contract: `PLAYBOOK-CLI-USAGE`): at exit `3` the output names which Work is waiting and for what. If it's waiting on an execution outcome and you are the ship agent for that Work, record it (protocol §3). If it's waiting on an assurance verdict and you are the verification agent, record it (protocol §4). If it's waiting on the *other* seat's input, you are done for now — never fill in the other seat's record yourself.
- **Re-running the same dispatch is safe (idempotent) and is the crash-recovery move** (full rationale: `PLAYBOOK-CLI-USAGE`). If you are unsure whether your last recording landed, the answer is always: record it (or re-record it — idempotent) and re-dispatch. Never invent a workaround to "force" a stuck-looking run past exit `3`; pending is correct until the real outcome is known and recorded.
...
## 7. Worked example — task-m1-003 (moved)

The real record sequence from `TASK-M1-003`'s own CLI-UX PR — the exact
ship/verify recording loop under push mode, walked fact-by-fact through
`.orc/task-m1-003.jsonl` — has moved to
`docs/reports/task-m1-003-worked-example.md` as historical reference
(`T6`: appended, not deleted; `TASK-M5-002`). Nothing about the loop it
describes has changed; read it there for the full trace.
...
- **`evidence_refs`** on the assurance verdict (`FACT-ASSURE-SETTLED`),
  for a verification agent's audit trail (§4 item 6 above).
...
- `docs/delivery/watchtower-operations.md` (`PLAYBOOK-WATCHTOWER`) — ship/verification-scout seats in the surrounding human-driven process
- `docs/adapters/omp/mapping.md` (`ADAPTER-OMP-MAPPING`) — the OMP-specific `executor-identity/v1` field mapping (§2 states the harness-independent shape only)
- `docs/reports/task-m1-003-worked-example.md` — the former §7 worked example, moved here as historical reference
