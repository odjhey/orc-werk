...
- **Outcomes are recorded into the config/backing store, not the journal directly.** In M1a/M1a+, the ExecutionPort and AssurancePort read their next-attempt outcome from the dispatch config's `attempts` entries (see `PLAYBOOK-CLI-USAGE`'s Config section). Prefer the `orc record` verb for either seat's recording — `orc record <run-id> --work <work-id> --verdict accepted|rejected ...` for the verify seat (issue #192), `orc record <run-id> --work <work-id> --outcome completed|failed ...` for the ship seat (its sibling; exactly one of `--verdict`/`--outcome` per invocation, `ERR-VALIDATION` otherwise — one seat, one recording): each validates the current requested attempt and performs the same merge-only config update atomically, then prints the exact resume command without running it. Dispatch remains a separate party's act under the one-party-dispatch rule. Hand-editing the persisted config remains legal and follows the identical semantics and discipline documented here—the verb is validation sugar, not a new recording or journal contract. In either path, the kernel journals the resulting facts itself via the normal observation path on the next `orc dispatch` (`SCN-007`); you never hand-author journal records.
