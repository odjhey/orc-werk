...
4. **Candidate-only blocking (issue #295).** The candidate you are verifying is the diff at the derived sha — never the PR's own mutable prose (title, description). Editing that prose does not, and cannot, change the fingerprint you derived. A defect confined to that prose (a stale claim, an already-fixed issue still described as open) is real and MUST be reported — prefix it `NON-BLOCKING` in `findings` — but it MUST NOT by itself turn an otherwise-earned `accepted` into `rejected`: rejecting there would burn an attempt budget re-verifying a diff you already confirmed is correct. `NON-BLOCKING` is not "ignore it" — the watchtower is required to fix the prose before merging (`PLAYBOOK-WATCHTOWER`'s merge gate); it is only not a reason to withhold acceptance of the diff.
