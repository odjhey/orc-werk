...
- **Config persistence, run-id-only re-dispatch (issue #55 H2).** On a run's first `dispatch`, the effective config is durably copied into that run's own directory, `<journal-dir>/<run_id>/config.json`. A later `dispatch` for the same run may omit `--config` entirely — `intent`/`--run-id` are still required by argparse, but the config itself resolves from the run dir (the intent text is ignored on any resumed dispatch anyway, per `Orchestrator.bootstrap`'s no-op-when-already-bootstrapped rule). An explicit `--config` on a later dispatch still wins and refreshes the persisted copy. `next:` re-dispatch affordances (`status`/`dispatch` output) name the durable in-run-dir config path once it exists, never the caller's own ephemeral `--config` path — a different reader (a fresh session, a different agent) may not have access to wherever the original `--config` file lived.
...
Every run created under this code writes the **new per-run directory layout**: `<journal-dir>/<run_id>/` holding `journal.jsonl` (the canonical `JournalPort` file), `times.jsonl` (observed-at sidecar, `CONTRACT-DURABILITY`), `reports.jsonl` (crew reports, `EXT-CREW-REPORT-V1`), `report.html` (this run's default `orc report` output), and `config.json` (the persisted effective dispatch config, issue #55 H2 — see the Config-persistence bullet above). Run lifecycle is directory lifecycle for this layout: a run "exists" once its directory does.

A **legacy flat `.orc` directory from before issue #55 keeps working unmodified** — every read path (bare `orc` index, `status`/`history` by bare run id, `report`/`report --index`/`--all`/`--match`, sidecar discovery) accepts both layouts. Writes never migrate a run: once a run has a legacy `<run_id>.jsonl` file, its journal keeps being read from and appended to that exact flat path for the rest of its life — a run's journal never splits across both layouts mid-run. This is decided independently per artifact (journal, times sidecar, crew-report log), each checking only its own legacy filename's existence — a run predating issue #55 can, for example, keep a flat journal while a crew report appended to it today still lands in the new per-run-directory layout, since `orc crew-report append` never requires a journal to exist at all.
...
|---|---|---|---|
| `no-mistakes` TOON output has no schema/version guard | `assurance.adapter: "no-mistakes"` (`TASK-M2-001`, `NoMistakesAssurance`) parses `axi status` TOON with a small, purpose-built tolerant parser (`orc_werk.adapters.no_mistakes.toon.parse_toon`), reverse-engineered from one CLI version's observed output. A future `no-mistakes` release that renames/reshapes fields (e.g. `run.status`, the `gate.findings` table columns) would silently parse incorrectly (missing fields read as absent) rather than fail loudly. | Re-probe `axi status`/`axi run` output shape against the installed `no-mistakes` version before upgrading it in an environment that uses this adapter; re-run `tests/conformance/test_no_mistakes_assurance_unit.py`'s `ParseToonTest` and the CONF-ASSURE suite. | Open (recorded at `TASK-M2-001` implementation time; no version pin/guard exists yet — see `docs/adapters/no-mistakes/mapping.md` "TOON parsing"/"Limitations"). |
