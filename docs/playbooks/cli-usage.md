...
orc record <run-id> --work <work-id> --outcome <completed|failed> [recording options]
                                                                   # verify seat's --verdict also accepts inconclusive (ADR-0006, SCN-021)
...
- **Assurance budget (`ADR-0006`, `SCN-021`).** `max_assurance_attempts` (config key; default `2`) bounds how many assurances one candidate may receive within one execution attempt. An `inconclusive` verdict within the budget re-requests assurance of the *same* candidate on the next dispatch (the run rests at `ASSURING`, exit `3`, `next:` naming the assurance index); exhausting it blocks the Work with `reason: assurance-inconclusive`. It never consumes `max_attempts`. Like `max_attempts`, it is journaled into `FX-CREATE-WORK` at creation and is single-authority thereafter: an explicit later value that disagrees is refused with `ERR-VALIDATION` (`SCN-008`'s match-or-refuse rule). A scripted config carries the additional assurance settlement(s) for the same attempt in an attempt entry's optional `assurances` array — the ordered sibling of `assurance`, consumed by `assurance_number` (element `0` *is* `assurance`; supplying both is `ERR-VALIDATION`). `orc record --verdict` writes the slot for the run's current assurance number, so an incremental run never has to hand-author the array.
...
- **Operator abandon (`TASK-M3B-001`, issues #76/#95/#289).** `--abandon-work <work_id> --abandon-reason "<why>" [--abandon-by "<who>"]` on `orc dispatch` records `DEC-ABANDON-ATTEMPT`/`FACT-ATTEMPT-ABANDONED` (`STATE-DELIVERY` item 9) for the named work, then continues the same dispatch pass — an ordinary retry or block follows immediately. Legal only when that work is currently resting at an unresolved candidate-observation conflict, at `EXECUTING` after a completed Execution settled with no bound Candidate, or at `ASSURING` with its current attempt still unsettled (exit `3`'s `pending, awaiting=assurance-verdict`) and the operator has out-of-band reason to believe the assurance cannot be trusted to settle correctly on its own — either it will never settle (issue #95's adapter-owned in-flight case), or the bound candidate has been externally invalidated (issue #289: a real change to the underlying subject the kernel has no affordance to detect while `ASSURING`; `orc dispatch`'s stderr divergence warning for a Git-backed candidate is advisory only, never a substitute for this operator judgment call) — anything else is rejected with `ERR-VALIDATION` and a `next` pointer at `orc status <run>`. Abandoning under the issue #289 basis preserves the frozen candidate binding and spends the full attempt cost exactly like the issue #95 basis: it is never a supersession of the stale candidate, never rebinds or replays it, and never waives the retry-budget arithmetic — a run already at `max_attempts` has no cheaper recovery than starting a new run. `--abandon-by` defaults to `$USER`/`whoami`; a flag, not a config-entry, is the chosen recording surface for this operator-only power (see the PR body's design rationale) — it is never available to the ship/verify agent seats `docs/playbooks/agent-cli-usage.md` governs. Like `orc cancel` immediately above, `--abandon-work` tolerates an invalid persisted config (`SCN-010` "Additional note", issue #236) with the same lenient-load-plus-stderr-note behavior; an ordinary `orc dispatch` of that same config (no `--abandon-work`) is still refused unchanged.
