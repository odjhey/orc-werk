...
| `orc refs --resolve`'s read-only allowlist assumes fixed provider CLI argv shapes at pinned versions | `orc_werk.cli.refs._vet_read_only` (`TASK-M3C-002`) vets every resolve command's argv by matching fixed, shape-based forms observed against one version each of `git`, `acpx`, `bd`, and `no-mistakes` (e.g. `git [-C <path>] show ...`, `acpx <agent> sessions <history\|show> ...`, `bd [--json] [-C <path>] <list\|show> ...`, `no-mistakes axi <status\|logs> ...`) — the same reverse-engineered-from-observed-output posture as the TOON parser above, not a spec these tools publish. A provider CLI release that reshapes one of these invocation forms (new required flag before the subcommand, a renamed subcommand, `git`/`bd`/`acpx` invoked via an absolute/relative path rather than the bare name this check matches) would either wrongly REFUSE a legitimate read-only command (safe failure mode -- degrades to the manual command, never executes) or, in the git/bd cases specifically, could in principle let a differently-shaped-but-still-mutating invocation slip past a check written against the old shape (unverified against any concrete case; the design intent is fail-closed, but the allowlist has not been adversarially re-probed against every provider version). | Re-probe each vetted tool's actual invocation shape against the installed version before relying on `--resolve`/`--resolve-all` in an environment that upgrades `git`/`acpx`/`bd`/`no-mistakes`; re-run `tests/scenarios/test_cli_refs.py`'s `VetReadOnlyUnitTest` (extend it with the new shape) and `RenderResolutionUnitTest`/`RefsResolveCliTest` before trusting execution again. | Open (recorded at `TASK-M3C-002` implementation time; no version pin/guard exists yet, matching the `no-mistakes` row above). |
