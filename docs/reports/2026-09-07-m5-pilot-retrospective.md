...
3. **Frozen-candidate gap (issue #289, filed 2026-09-07, `OPEN` at anchor).** A ship candidate can freeze at `orc record --outcome` and enter `ASSURING`, and the ledger has no affordance to rebind that candidate if a *sibling* run's settlement invalidates a present-tense claim in it seven minutes later — `record_execution_outcome_entry` (`src/orc_werk/cli/config.py:1259-1264`) rejects a second recorded outcome unconditionally, by design: rebinding a frozen Candidate would violate evidence-candidate binding (`INV-007`, `INV-008`), not the assurance re-request budget (`INV-021`, which an earlier draft of this line mis-cited here; corrected 2026-09-08 per issue #289's own accepted citation correction). The only two options inside the run are to let a now-inaccurate candidate go to verify, or force a reject and burn the attempt budget on a candidate that was correct when frozen. The sanctioned recovery — operator `--abandon-work`, re-ship under a new run on the same branch/PR — works, but it is not something the seat model resolves without an operator stepping in. This is a real, still-open product gap, not a one-off; issue #289 proposes a first-class "superseded-by-external-change" affordance as the fix.
4. **`V7` model-family pairing did surface friction.** Two `openai-codex/gpt-5.6-sol` verify spawns against `m5-omp-harness-pilot`'s `docs` work died within seconds of `usage_limit_reached` on 2026-09-06 and recorded nothing to the ledger (`docs/delivery/seat-reliability.md:21-50`); frontmatter model-list order is resolution preference only, and OMP did not fall through to the list's second entry at spawn time on a usage-limit error, so both dead spawns had to be hand-substituted to `google-antigravity/gemini-3.8-flash`, which then completed and was accepted. `ADR-0007`'s own `V7` amendment (`docs/decisions/ADR-0007-omp-primary-harness.md:386-399`) responds directly to this: it drops the specific `openai-codex`-vs-`anthropic` family assignment as a contract-layer requirement, keeping only the underlying invariant — verify runs on a family different from whichever family ship actually used — and explicitly declines to re-pin `.omp/agents/verify.md`'s own model list itself. That list was already reordered `google-antigravity/gemini-3.8-flash` first, `openai-codex/gpt-5.6-sol` second, by PR #268, ahead of this amendment landing (`ADR-0007:393-395`); the amendment leaves the ordering as the deployment-preference decision it always was and requires only that it keep satisfying the different-family invariant. The invariant itself was never violated (every observed verify spawn ran non-Anthropic against an Anthropic ship), but the specific deployment preference did not survive contact with quota reality.
...
> "`TASK-M5-008`'s retrospective needed one figure: how many settled verify verdicts exist, by model family. There is no command that answers this. So three separate **ship** seats each hand-wrote a `jq`/Python join over `.orc/*/journal.jsonl` x `times.jsonl`, and each produced a different wrong answer:" (issue #285, line 5; emphasis on "ship" added — the source is not bolded there). Its own table (issue #285, lines 7–11) compares claimed-vs-true whole-ledger counts across three commits, not three write-up attempts: two *different* write-up attempts (1, `0584a66`; 2, `645c2c4`) and one ADR-sibling commit (`f1c3d6a`) that ran the same hand-rolled join outside this write-up — and its systematic-failure account (issue #285, line 23) states the recurring pattern plainly: "a seat writes a number, the ledger settles underneath it, and the claim is false by the time CI goes green."
...
**Corrected here, 2026-09-08, issue #285:** the paragraph above is itself the defect class it describes. Its `glob.glob(".orc/*/journal.jsonl")` (`:173`) silently omits every legacy flat-layout run (`.orc/<run>.jsonl` + `.orc/<run>+times.jsonl`) that `orc_werk.adapters.jsonl.layout.discover_run_ids` includes — the same canonical-discovery gap `orc`'s bare index and every other whole-ledger listing already close. Re-run through the shipped `orc census --as-of 2026-09-07T12:11:25Z` verb (`TASK-FIX-285`), which enumerates both layouts, the true whole-ledger figure at this document's own anchor is `{'accepted': 131, 'rejected': 50, 'inconclusive': 0}` (181 total, not 173) — short by exactly the 8 accepted verdicts recorded only in legacy-layout runs — plus 13 further settled verdicts with no times-sidecar entry at all, dated-excluded from this (or any) as-of window and reported separately by the verb as `undated`. The original `{'accepted': 123, 'rejected': 50, 'inconclusive': 0}` figure is left unedited above, not because it was correct, but because this document's own §4 thesis is that a fixed prose number must never be silently overwritten — only re-derived and dated. `orc census` is now the durable, re-runnable source for this question; hand-rolled joins over `.orc/*/journal.jsonl` should not be written again.
...
- Issue #293 (`orc cancel` reaches a terminal state with no execution outcome and no caller-role authorization, weakening the record-before-yield rung) and issue #288 (operator-abandon vs. retry-exhaustion indistinguishable in the projection) — both open product findings adjacent to this milestone's seat mechanics, neither blocking `TASK-M5-005`'s own closure. Per-process rate-limit / usage-limit scoping (the `V7` fallthrough gap, §2 item 4) is not tracked by any filed issue — it is recorded only in `docs/delivery/seat-reliability.md:21-50` and `ADR-0007`'s `V7` amendment (`docs/decisions/ADR-0007-omp-primary-harness.md:386-399`).
- Bun-test coverage for `.omp/extensions/*` — not applicable post-retirement: the hook file never landed on `master` in the first place, so there is no `.omp/extensions/*` in the delivered tree for a Bun test to cover (it lived only on the closed-unmerged `task-m5-005-orc-seat-hook` branch, PR #284, preserved at head `69e4829`).
