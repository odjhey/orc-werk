...
> "`TASK-M5-008`'s retrospective needed one figure: how many settled verify verdicts exist, by model family. There is no command that answers this. So three separate **ship** seats each hand-wrote a `jq`/Python join over `.orc/*/journal.jsonl` x `times.jsonl`, and each produced a different wrong answer:" (issue #285, line 5; emphasis on "ship" added — the source is not bolded there). Its own table (issue #285, lines 7–11) compares three *different* write-up attempts' claimed-vs-true whole-ledger counts, and its systematic-failure account (issue #285, line 23) states the recurring pattern plainly: "a seat writes a number, the ledger settles underneath it, and the claim is false by the time CI goes green."
