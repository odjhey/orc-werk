...
def build_parser() -> argparse.ArgumentParser:
    ...
    refs_parser = subparsers.add_parser(
        ...
        "evidence_refs, candidate identity, landing (derived from a gh-pr:N artifact/evidence ref, "
        "e.g. via 'orc record --evidence-ref gh-pr:N'; issue #65 -- never journaled itself, the "
        "merge/PR lives in the forge), the Beads mirror when configured), each indexed and shown "
        "with a resolve command. --resolve/--resolve-all (TASK-M3C-002) execute that SAME command "
        "-- never a second command vocabulary -- vetted read-only at construction (cat; git "
        "show/--stat; bd list/show; historical providers' acpx sessions history/show and axi "
        "status/logs resolvers for journals predating 0.5.0; gh pr view <N> --json <fields> for a "
        "landing row -- nothing else); "
